...
pub fn run(year: &Option<usize>) {
    ...
    if unused_languages.is_empty() {
        ...
        match year {
            Some(year) => println!("Unused languages for year {}: {}", year, unused_languages.join(", ")),
            None => println!("Unused languages: {}", unused_languages.join(", ")),
        }
    }
}
                ...
                for day_entry in day_entries.flatten() {
                    if let Ok(day_type) = day_entry.file_type() {
                        if day_type.is_dir() {
                            ...
                            let day_languages = get_language_directories(&day_path.to_str().unwrap());
